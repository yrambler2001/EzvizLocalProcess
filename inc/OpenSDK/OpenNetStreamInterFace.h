...
OPENSDK_API OPENSDK_RESULT CALLBACK OpenSDK_StartRealPlay_gbCode(const char* szSessionId, HWND hPlayWnd, const char* szDevice_id, const char* szChannel,const char* szPlatform_id,const char* szBizType,const char* szSafeKey);

/** 
 *  \brief 针对摄像头进行预览, 指定主\子码流，不适用于国标设备
           异步接口，返回值只是表示操作成功，不代表播放成功
           如果接口返回-1, 结果根据消息回调函数msgid判断, 若msgid=INS_PLAY_START, 表示成功, 若msgid=INS_PLAY_EXCEPTION, 表示失败
 *  \param[in] szSessionId   会话ID
 *  \param[in] hPlayWnd      播放窗口句柄, 如果窗口句柄为NULL表示纯粹取流，不做播放
...
/** 
 *  \brief 录像搜索，包含云存储和设备SD卡录像, 如果关联NVR优先搜索NVR录像，不适用于国标设备
...
/** 
 *  \brief 按录像源进行搜索，支持本地录像和云存储录像搜索,搜索本地录像时如果关联NVR优先搜索NVR录像，不适用于国标设备,支持长序列号设备搜索
           同步接口,不适用于国标设备
 ...
 *  时间格式为:yyyy-MM-dd hh:mm:ss
 */
OPENSDK_API OPENSDK_RESULT CALLBACK OpenSDK_StartSearchRecord(const char* szSessionId, const char* szDevSerial,const char* szChannel, const char* szPlatform_id, const char* szBizType,const char* szStartTime, const char* szStopTime, const int iSearchType);
 ...
 *  \param[in] szChannel     国标设备通道号
 *  \param[in] szPlatform_id 国标设备通道号
 ...
 *  时间格式为：yyyy-MM-dd hh:mm:ss
 */
OPENSDK_API OPENSDK_RESULT CALLBACK OpenSDK_StartPlayBack_gbCode(const char* szSessionId, HWND hPlayWnd, const char* szDevice_id, const char* szChannel,const char* szPlatform_id,const char* szBizType, const char* szStartTime, const char* szStopTime,const char* szSafeKey);
 ...
 */
OPENSDK_API OPENSDK_RESULT CALLBACK OpenSDK_SetPlayBackScale(const char* szSessionId,int iScale,const char* szOsdTime = NULL);
...
/** 
 *  \brief  注销接口, 同OpenSDK_Mid_Login接口配对使用，不支持国标设备
 *  \sa OpenSDK_Mid_Login之后调用
 */
...
*}
*/
OPENSDK_API OPENSDK_RESULT CALLBACK OpenSDK_Data_GetDeviceList(const char* szBizType, int iPageStart, int iPageSize, void** pBuf, int* iLength);

/** 
 *  \brief 获取他人分享的摄像头列表，不支持国标设备
 *  \param[in]  iPageStart    分页起始页，从0开始
 *  \param[in]  iPageSize     分页大小, 限制最大1000,默认100, 建议不超过100
 *  \param[out] pBuf          摄像头列表的JSON字符串
 *  \param[out] iLength       获取到的数据大小
...
/** 
 *  \brief 获取设备详细信息, 包括预览能力级, PTZ, 对讲能力级等,不支持国标设备
 *  \param[in]  szDevSerial     设备序列号
 *  \param[in]  iChannelNo      设备通道号
 ...
 *      "ptz_preset":1,                     // 是否支持云台预置点:1-支持, 0-不支持
        "support_quickplay_way":1           // 设备是否支持流媒体倍速回放:1-支持, 0-不支持
 ...
 *  \return 0表示成功，其他值表示失败
 */
OPENSDK_API OPENSDK_RESULT CALLBACK OpenSDK_Data_GetDeviceDetailInfo(const char* szDevSerial, const char* szChannel, const char* szBizType, const char*szPlatform_id, const bool bUpdate, void** pDevDetailInfo, int* iLength);
...
/** 
 *  \brief 解密告警图片(建议加密的图片才调用，非加密图片直接下载，可以通过url里面isEncrypted=1来区分),,不支持国标设备
 *  \param[in] szAccessToken 认证Token
...
/** 
 *  \brief 开启接收推送, 不推荐应用在服务器端上,不支持国标设备
